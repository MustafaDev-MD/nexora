...
function measure(){
  ...
  if(tl){ tlTop = pageTop(tl); tlH = tl.offsetHeight || 1; }
  if(hero){ heroTop = pageTop(hero); heroH = hero.offsetHeight || 1; }
  if(orbAnchor){
...
function carousel(rootEl, opts){
  if(!rootEl) return;
  opts = opts || {};
  var track = $('.track', rootEl); if(!track) return;
  var slides = $$('.slide', track); if(!slides.length) return;
  var i = 0, timer = null, hover = false, dots = [];
...
function initGL(){
  ...
  var cv = $('#gl'), renderer;
  try{ renderer = new THREE.WebGLRenderer({canvas:cv, alpha:true, antialias: S.vw > 800, powerPreference:'high-performance'}); }catch(e){ return null; }
  ...
  var rings = ringCfg.map(function(c){
    ...
    var tube = new THREE.Mesh(new THREE.TorusGeometry(c.R, .009, 6, 120), reg(new THREE.MeshBasicMaterial({color:c.c2, transparent:true, opacity:.55, blending:THREE.AdditiveBlending, depthWrite:false}), .55));
    g.add(tube);
    orb.add(g); return {g:g, sp:c.sp};
  });

  /* round particle sprite (avoids square points) */
  var pcDot = doc.createElement('canvas'); pcDot.width = pcDot.height = 64;
  var pctx = pcDot.getContext('2d');
  var prg = pctx.createRadialGradient(32,32,0,32,32,32);
  prg.addColorStop(0,'rgba(255,255,255,1)');
  prg.addColorStop(.35,'rgba(255,255,255,.85)');
  prg.addColorStop(.7,'rgba(255,255,255,.25)');
  prg.addColorStop(1,'rgba(255,255,255,0)');
  pctx.fillStyle = prg; pctx.fillRect(0,0,64,64);
  var pMap = new THREE.CanvasTexture(pcDot);
  ...
  var pts = new THREE.Points(pg, reg(new THREE.PointsMaterial({
    map: pMap,
    color: 0xbcd2ff,
    size: 2.4,
  ...
  function resize(){
    var dpr = Math.min(window.devicePixelRatio || 1, S.vw < 900 ? 1.25 : 1.75);
    ...
    var small = S.vw < 700;
    /* same look in light + dark (no theme recolor) */
    shapes.forEach(function(s){
      s.mat.opacity = small ? .55 : .92;
      if(s.mat.emissive) s.mat.emissiveIntensity = 0.35;
      s.g.children.forEach(function(ch){
        if(ch.material && ch.material.isLineBasicMaterial){
          ch.material.color.setHex(0xa5dcff);
          ch.material.opacity = 0.6;
        }
      });
      s.g.userData.off = small && !s.d.keep;
    });
    if(typeof pts !== 'undefined' && pts && pts.material){
      pts.material.color.setHex(0xbcd2ff);
      pts.material.opacity = 0.85;
      pts.material.size = 2.4;
    }
  }
...
function frame(now){
  ...
  if(tl && steps.length){
    ...
    if(tpv !== tlLast){
      ...
      steps.forEach(function(st, idx){ st.classList.toggle('lit', tpv >= (idx/(Math.max(steps.length-1,1)))*.97 - .001 && tpv > 0.02); });
    }
  }
...
setTimeout(hidePreloader, reduce ? 100 : 400);
...
/* ---------- boot ---------- */
var navTick = function(){ if(nav) nav.classList.toggle('scrolled', (window.scrollY||0) > 24); spy(); schedule(); };
...
function boot(){
  ...
  GL = initGL();
  try{
    new MutationObserver(function(){ if(GL) GL.resize(); }).observe(root, {attributes:true, attributeFilter:['data-theme']});
  }catch(e){}
